...
import { ResumeSourcePicker, ResumeSource } from './ResumeSourcePicker';
import type { BaseResumeSummary } from '@/types/documents';

export type FormatterMode = 'optimize' | 'coverLetter';
...
export function OptimizeForm({
  ...
  const submitLabel = isCoverLetter ? 'Generate cover letter' : 'Score my resume';
  const busyLabel = isCoverLetter ? 'Generating…' : 'Scoring…';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label htmlFor="jobTitle" className="eyebrow mb-2 block">
            Job title{' '}
            <span className="opacity-60">{isCoverLetter ? '(recommended)' : '(optional)'}</span>
          </label>
          <input
            id="jobTitle"
            name="jobTitle"
